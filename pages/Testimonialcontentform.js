...
function Testimonialcontentform() {
  ...
  const ratinglastvalue = useSelector(state => state.rating.slice(-1));
    //console.log(ratinglastvalue);
    let ratingtruefalse = ratinglastvalue[0];
    //console.log(ratingtruefalse.checked);
    let valuetruefalse = ratingtruefalse.checked;
    console.log(valuetruefalse);
  ...
  const adddata = () => {    
    ...
    // console.log(checked)
    if (  heading && description && name && uploadedFile) {
      dispatch(
        formAdd({
          id: nanoid(),                 
          rating ,
